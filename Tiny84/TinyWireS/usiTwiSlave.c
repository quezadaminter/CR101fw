...
#  define USI_START_COND_INT  USISIF
#  define USI_START_VECTOR    USI_STR_vect// USI_START_vect
  ...
  switch ( overflowState )
  {
    ...
    case USI_SLAVE_CHECK_ADDRESS:
      if ( ( USIDR == 0 ) || ( ( USIDR >> 1 ) == slaveAddress) )
      {
        if ( USIDR & 0x01 )
        {
          overflowState = USI_SLAVE_CHECK_REPLY_FROM_SEND_DATA;//USI_SLAVE_SEND_DATA;
          //USI_REQUEST_CALLBACK(); // Load a byte from the user code.
        }
        else
        {
